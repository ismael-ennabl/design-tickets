...
#   ./publish.sh DSGN-1126              (if youtrack/DSGN-1126/ already exists)
...
REPO_ROOT="$(dirname "$0")"
DEST_DIR="$REPO_ROOT/youtrack/$TICKET_ID"

# ── Ingest source ─────────────────────────────────────────────────────────────
if [ -z "$SRC" ]; then
  # No source given — assume folder already exists (extracted manually)
  if [ ! -d "$DEST_DIR" ]; then
    echo "Error: no source provided and youtrack/$TICKET_ID/ does not exist."
    exit 1
  fi
  echo "Using existing youtrack/$TICKET_ID/"
  ...
  echo "Extracted zip to youtrack/$TICKET_ID/"

elif [ -d "$SRC" ]; then
  # Directory — copy whole folder
  mkdir -p "$DEST_DIR"
  cp -r "$SRC/." "$DEST_DIR/"
  echo "Copied directory to youtrack/$TICKET_ID/"

elif [ -f "$SRC" ]; then
  # Single HTML file
  mkdir -p "$DEST_DIR"
  cp "$SRC" "$DEST_DIR/index.html"
  echo "Copied design to youtrack/$TICKET_ID/index.html"
...
PUBLIC_URL="https://$GH_USER.github.io/$GH_REPO/youtrack/$TICKET_ID/"
echo ""
